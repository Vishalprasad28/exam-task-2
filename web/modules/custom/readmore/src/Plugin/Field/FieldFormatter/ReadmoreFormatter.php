...
class ReadmoreFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['trim_by_word'] = [
      '#type'          => 'checkbox',
      '#title'         => $this->t('Trim by word'),
      '#default_value' => $this->getSetting('trim_by_word'),
    ];
  ...
  public function settingsSummary() {
    ...
    elseif (!empty($settings['trim_length'])) {
      if (!empty($settings['trim_by_word'])) {
        $summary[] = $this->t('Text trimmed to @limit words', [
          '@limit' => $settings['trim_by_word'],
        ]);
      }
      else {
